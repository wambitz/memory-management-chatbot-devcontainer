...
bool ChatBotApp::OnInit() {
    // Get the executable path and construct data paths
    wxFileName execPath(wxStandardPaths::Get().GetExecutablePath());
    wxString execDir = execPath.GetPath();
    dataPath = std::string(execDir.mb_str()) + "/../";
...
ChatBotPanelDialogItem::ChatBotPanelDialogItem(wxPanel* parent, wxString text, bool isFromUser)
    ...
    _chatBotTxt = new wxStaticText(
        this, wxID_ANY, text, wxPoint(-1, -1), wxSize(150, -1), wxALIGN_CENTRE | wxBORDER_NONE);
